...
from monkq.utils.dataframe import kline_1m_to_freq, kline_indicator, kline_dataframe_window
from monkq.assets.instrument import Instrument, FutureInstrument
...
class Initer:
    def __init__(self, context: Context):
        ...
        self._cache_kline: Dict[str, Dict[str, pandas.DataFrame]] = {}
        self._cache_indicator: Dict[str, Dict[str, pandas.DataFrame]] = {}
    ...
    def init_indicator(self, freq: str, instrument: INSTRUMENT, func: str,
                       columns: List[str], *args, **kwargs) -> None:
        if freq == '1min':
            exchange = self.context.exchanges[instrument.exchange.name]
            data = exchange.all_data(instrument)
        else:
